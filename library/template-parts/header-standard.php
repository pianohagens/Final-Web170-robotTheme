...
} else if ( get_theme_mod( 'robotTheme-logo', '' ) != '' ) {
	$logo = "<a href=\"". esc_url( home_url( '/' ) ) ."\" title=\"". esc_attr( get_bloginfo( 'name', 'display' ) ) ."\"><img src=\"". esc_url( get_theme_mod( 'robotTheme-logo', '' ) ) ."\" alt=\"". esc_attr( get_bloginfo( 'name' ) ) ."\" /></a>";
}
?>
...
<div class="site-container">
    ...
    <div class="site-header-right">
        ...
        if ( robotTheme_is_woocommerce_activated() && get_theme_mod( 'robotTheme-header-shop-links', true ) ) { ?>
            ...
            <div class="header-cart">
                <a class="header-cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'robotTheme'); ?>">
 ...
 <?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'robotTheme'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?>
	...
	<?php if( get_theme_mod( 'robotTheme-header-search', true ) ) : ?>
...
} else if ( is_front_page() && get_theme_mod( 'robotTheme-slider-type', 'robotTheme-no-slider' ) == 'robotTheme-no-slider' && !get_header_image() ) {
	$is_translucent = false;
} else if ( is_front_page() && get_theme_mod( 'robotTheme-slider-type', 'robotTheme-no-slider' ) == 'robotTheme-slider-plugin' && get_theme_mod( 'robotTheme-slider-plugin-shortcode', '' ) == '' ) {
	$is_translucent = false;
}
?>
...
<nav id="site-navigation" class="main-navigation border-bottom <?php echo ( $is_translucent ) ? sanitize_html_class( 'translucent' ) : sanitize_html_class( '' ); ?>" role="navigation">
	...
	<div id="main-menu" class="main-menu-container robotTheme-mobile-menu-standard-color-scheme">
